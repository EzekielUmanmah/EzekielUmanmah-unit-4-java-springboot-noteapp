const cookieArr = document.cookie.split('=');
const userId = cookieArr[1];

const submitForm = document.getElementById('note-form');
const noteContainer = document.getElementById('note-container');

const noteBody = document.getElementById('note-body');
const updateNoteBtn = document.getElementById('update-note-button');

const headers = {
  'Content-Type': 'application/json',
};

const baseURL = 'http://localhost:4000/api/v1/notes';

function handleLogout() {
  let c = document.cookie.split(';');
  for (let i in c) {
    document.cookie =
      /^[^=]+/.exec(c[i])[0] + '=;expires=Thu, 01 Jan 1970 00:00:00 GMT';
  }
}

const handleSubmit = async (e) => {
  e.preventDefault();
  const body = {
    body: document.getElementById('note-input').value,
  };
  await addNote(body);
  document.getElementById('note-input').value = '';
};

async function addNote(note) {
  const res = await fetch(`${baseURL}/user/${userId}`, {
    method: 'POST',
    body: JSON.stringify(note),
    headers,
  }).catch((err) => console.log('Error, cannot add note: ', err.message));

  if (res.status === 200) {
    return getNotes(userId);
  }
}

async function getNotes(userId) {
  await fetch(`${baseURL}/user/${userId}`, {
    method: 'GET',
    headers,
  })
    .then((res) => res.json())
    .then((data) => createNoteCards(data))
    .catch((err) => console.log('Could not fetch notes...:', err));
}

async function getNoteById(noteId) {
  await fetch(`${baseURL}/${noteId}`, {
    method: 'GET',
    headers,
  })
    .then((res) => res.json())
    .then((data) => populateModal(data))
    .catch((err) =>
      console.log('Could not fetch note for modal...:', err.message)
    );
}

async function handleUpdateNote(noteId) {
  const body = {
    id: noteId,
    body: noteBody.value,
  };

  await fetch(baseURL, {
    method: 'PUT',
    body: JSON.stringify(body),
    headers,
  }).catch((err) => console.log('Could not update note...:', err.message));

  return getNotes(userId);
}

async function deleteNote(noteId) {
  await fetch(`${baseURL}/${noteId}`, {
    method: 'DELETE',
    headers,
  }).catch((err) => console.log('Could not delete note...:', err));

  return getNotes(userId);
}

const createNoteCards = (data) => {
  noteContainer.innerHTML = '';

  data.forEach((note) => {
    let noteCard = document.createElement('div');
    noteCard.classList.add('m-2');
    noteCard.innerHTML = `
      <div class='card d-flex' style='width: 18rem; height: 18rem;'>
      <div class='card-body d-flex flex-column justify-content-between' style='height: available'>
        <p class='card-text'>${note.body}</p>
        <div class='d-flex justify-content-between'>
          <button class='btn btn-danger' onclick="deleteNote(${note.id})">Delete</button>
          <button onclick="getNoteById(${note.id})" type='button' class='btn btn-primary' data-bs-toggle='modal' data-bs-target='#note-edit-modal'>Edit</button>
          </div>
        </div>
      </div>
      `;

    noteContainer.append(noteCard);
  });
};

const populateModal = (note) => {
  noteBody.innerText = '';
  noteBody.innerText = note.body;
  updateNoteBtn.setAttribute('data-note-id', note.id);
};

getNotes(userId);

submitForm.addEventListener('submit', handleSubmit);

updateNoteBtn.addEventListener('click', (e) => {
  const noteId = e.target.getAttribute('data-note-id');
  handleUpdateNote(noteId);
});
